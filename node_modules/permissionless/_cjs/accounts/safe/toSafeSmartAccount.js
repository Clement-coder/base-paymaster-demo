...
const encode7579Calls_1 = require("../../utils/encode7579Calls.js");
const multiSendAbi = [
    {
        inputs: [
            {
...
const getInitializerCode = async ({ owner, safeModuleSetupAddress, safe4337ModuleAddress, multiSendAddress, safeSingletonAddress, erc7579LaunchpadAddress, setupTransactions = [], safeModules = [], validators = [], executors = [], fallbacks = [], hooks = [], attesters = [], attestersThreshold = 0, paymentToken = viem_1.zeroAddress, payment = BigInt(0), paymentReceiver = viem_1.zeroAddress }) => {
...
async function toSafeSmartAccount(parameters) {
    const { client, owners, address, version, safe4337ModuleAddress: _safe4337ModuleAddress, safeProxyFactoryAddress: _safeProxyFactoryAddress, safeSingletonAddress: _safeSingletonAddress, erc7579LaunchpadAddress, saltNonce = BigInt(0), validUntil = 0, validAfter = 0, nonceKey, paymentToken, payment, paymentReceiver } = parameters;
    const localOwner = await (0, utils_2.toOwner)({ owner: owners[0] });
    const entryPoint = {
    ...
    return (0, account_abstraction_1.toSmartAccount)({
        ...
        async encodeCalls(calls) {
            ...
            if (erc7579LaunchpadAddress) {
                const safeDeployed = await (0, utils_2.isSmartAccountDeployed)(client, await this.getAddress());
                if (!safeDeployed) {
                    const initData = get7579LaunchPadInitData({
                        safe4337ModuleAddress,
                        safeSingletonAddress,
                        ...
                        args: [
                            {
                                ...
                                callData: (0, encode7579Calls_1.encode7579Calls)({
                                    mode: {
                ...
                return (0, encode7579Calls_1.encode7579Calls)({
                    mode: {
        ...
        async signMessage({ message }) {
            ...
            return adjustVInSignature("eth_sign", await localOwner.signMessage({
                message: {
        ...
        async signTypedData(typedData) {
            return adjustVInSignature("eth_signTypedData", await localOwner.signTypedData({
                domain: {
                    chainId: await getMemoizedChainId(),
                    verifyingContract: await this.getAddress()
                },
